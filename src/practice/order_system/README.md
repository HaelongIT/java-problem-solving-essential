...
[Service: OrderService] (Implementation: 비즈니스 로직) ◀───┘
      │
      ▼
[Mapper: OrderMapper] (MyBatis 스타일 - 데이터 조회/Hash 연습)
      │
      ▼
...
├── controller
│   └── OrderController.java    # API 진입점
├── mapper
│   └── OrderMapper.java        # 미션 2: HashMap 활용 데이터 검색 (MyBatis 스타일)
├── service
...
- **포인트**: `split()`, `substring()`, `Integer.parseInt()` 등을 활용하여 지저분한 데이터를 정제합니다. 상품 목록은 콤마(,)로 구분되며 **여러 개**일 수 있으므로 반복 처리가 필요합니다.

### 2단계: 데이터 조회 최적화 (MyBatis Style Hash Practice)
- **대상 파일**: `mapper/OrderMapper.java`
- **미션**: `selectUserById`, `selectProductById` 메서드 내부에 `HashMap` 조회 로직을 작성하여 데이터를 즉시 찾을 수 있게 구현하세요.
- **포인트**: MyBatis XML의 `id`와 매칭되는 메서드 명칭을 사용하며, `Map.get()`으로 O(1) 탐색 감각을 익힙니다.
